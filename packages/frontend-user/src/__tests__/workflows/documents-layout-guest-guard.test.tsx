...
describe('documents layout guest workspace guard', () => {
  ...
  it('keeps normal users in the documents workspace', async () => {
    ...
    await user.type(screen.getByLabelText(/first name/i), 'Dashboard');
    await user.type(screen.getByLabelText(/last name/i), 'Writer');
    await user.click(screen.getByRole('button', { name: /^save$/i }));

    await waitFor(() => {
      expect(mockUpdateUser).toHaveBeenCalledWith({ firstName: 'Dashboard', lastName: 'Writer' });
    });
  });
});
