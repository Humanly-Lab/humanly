...
export default function EnrollmentSubmissionsPage() {
  ...
  const fetchEnrollmentAndSubmissions = useCallback(async () => {
    try {
      ...
      const selectedEnrollment = enrollmentsResponse.data.enrollments.find(
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-medium">User Submissions</h1>
        <p className="text-muted-foreground mt-2">
          {enrollment ? enrollment.email : 'Loading user'} submissions for this
          task.
        </p>
      </div>
        ...
        <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                The most recent valid submission is shown by default.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {latestSubmission ? (
                <div
                  ...
                  onClick={() =>
                    router.push(
                      `/tasks/${taskId}/submissions/${latestSubmission.id}`
                    )
                  }
                >
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Badge variant="outline">
                        {formatAttemptLabel(latestSubmission.attemptNumber)}
                      </Badge>
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      <FileText className="h-4 w-4 text-muted-foreground" />
                      <span>
                        {latestSubmission.documentTitle ||
                          enrollment.documentTitle ||
                          'Submission Document'}
                      </span>
                    </div>
                ...
                <div className="h-[180px] flex items-center justify-center rounded-md border">
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      This user has not submitted a document for this task.
                    </p>
                  </div>
                </div>
              )}
            ...
            <CardHeader>
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <CardDescription>
                    Historical submissions remain available for audit.
                  </CardDescription>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={fetchEnrollmentAndSubmissions}
                  disabled={isLoading}
                >
                    ...
                    <TableBody>
                      {submissions.map((submission, index) => (
                        <TableRow
                          ...
                          onClick={() =>
                            router.push(
                              `/tasks/${taskId}/submissions/${submission.id}`
                            )
                          }
                        >
                          <TableCell>
                          ...
                          <TableCell>
                            <Badge
                              variant={
                                index === 0 || submission.status === 'active'
                                  ? 'default'
                                  : 'secondary'
                              }
                            >
