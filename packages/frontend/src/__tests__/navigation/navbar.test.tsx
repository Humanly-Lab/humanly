...
describe('admin navbar', () => {
  ...
  it('opens My Account settings and saves the first and last name', async () => {
    mockUser = {
      email: 'admin@mail.com',
      name: 'Admin One',
      firstName: 'Admin',
    ...
    await user.type(firstName, 'Admin');
    await user.clear(lastName);
    await user.type(lastName, 'Two');
    await user.click(screen.getByRole('button', { name: /^save$/i }));

    await waitFor(() => {
      expect(mockUpdateUser).toHaveBeenCalledWith({ firstName: 'Admin', lastName: 'Two' });
    });
  });
