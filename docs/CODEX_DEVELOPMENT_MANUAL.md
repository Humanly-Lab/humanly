...
- If one PR closes multiple issues, list each issue and explain why one PR is
  safer than several.
- After every PR is merged, delete the corresponding feature/hotfix branch.
  Delete the remote branch on GitHub and delete the local branch when it is safe
  to do so. If a local branch is checked out by another worktree, still delete
  the remote branch and report the local worktree constraint explicitly.
