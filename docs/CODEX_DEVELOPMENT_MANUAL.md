...
- Backend logic: targeted backend tests, then broader tests when the surface is
  shared.
- Frontend visible UX: local mock browser smoke in `docs/LOCAL_DEV.md`.
- AI/provider behavior: real model smoke with trace capture and manual judgment
  when prompt/tool behavior matters.
