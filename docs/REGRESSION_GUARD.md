...
1. `pnpm lint` for package type/lint gates.
...
6. Package-specific tests when a package exposes a real `test` script; do not
   treat `pnpm --recursive --if-present test` as evidence unless output shows
   actual tests ran.
7. `.agents/skills/humanly-browser-e2e/SKILL.md` plus
   `docs/testing/BROWSER_E2E_PLAYBOOK.md` for browser-agent-assisted
   human-visible flows.
8. Post-deploy canary: a short human or agent-run smoke on app/admin.
