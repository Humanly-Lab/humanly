...
| #751 | `Single durable attempt` tasks allowed the same logged-in or guest task attempt to be manually submitted more than once, generating duplicate submissions/certificates. | `type:new-bug` | current QA batch | Backend submit path enforces one active submission per `task_attempt_id`; production smoke retests logged-in and guest duplicate submit behavior. |
