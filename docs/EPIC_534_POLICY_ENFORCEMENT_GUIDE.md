...
- Do not add backend-owned provider keys for product AI calls. Humanly stores
  and routes user-owned provider keys only.
- Policy enforcement for the current release is in-band with the selected
  user-owned provider configuration. Any future server-funded guard model needs
  a separate product/security decision before implementation.
- Trust-critical behavior must stay explicit in the active writing policy and
  must not silently fallback to hidden developer credentials.
...
Commands: `pnpm build:all`, `pnpm lint`, and the relevant existing QA harness
for the touched surface, such as `pnpm qa:backend:contract`,
`pnpm qa:ai:usage`, or `pnpm qa:deploy:smoke`. CI green before PR review.
